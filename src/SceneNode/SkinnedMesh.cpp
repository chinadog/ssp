...
: SkinningBuffers(0), AnimationFrames(0.f), FramesPerSecond(25.f),
    ...
    InterpolationMode(scene::EIM_LINEAR),
...
u32 SkinnedMesh::getFrameCount() const
{
...
scene::IMesh* SkinnedMesh::getMesh(s32 frame, s32 /*detailLevel*/, s32 /*startFrameLoop*/, s32 /*endFrameLoop*/)
{
...
void SkinnedMesh::skinMesh()
{
    ...
    if (!HardwareSkinning)
    {
        ...
        for (i=0; i<AllJoints.size(); ++i)
        {
            ...
            {
                scene::SSkinMeshBuffer* Buffer=(*SkinningBuffers)[ AllJoints[i]->AttachedMeshes[j] ];
        ...
        for (i=0; i<SkinningBuffers->size(); ++i)
            (*SkinningBuffers)[i]->setDirty(scene::EBT_VERTEX);
    }
    updateBoundingBox();
}
...
scene::E_ANIMATED_MESH_TYPE SkinnedMesh::getMeshType() const
{
    return scene::EAMT_SKINNED;
}
...
const c8* SkinnedMesh::getJointName(u32 number) const
{
...
s32 SkinnedMesh::getJointNumber(const c8* name) const
{
    for (u32 i=0; i<AllJoints.size(); ++i)
    {
...
scene::IMeshBuffer* SkinnedMesh::getMeshBuffer(u32 nr) const
{
...
scene::IMeshBuffer* SkinnedMesh::getMeshBuffer(const video::SMaterial &material) const
{
...
void SkinnedMesh::setMaterialFlag(video::E_MATERIAL_FLAG flag, bool newvalue)
{
    for (u32 i=0; i<LocalBuffers.size(); ++i)
...
void SkinnedMesh::setHardwareMappingHint(scene::E_HARDWARE_MAPPING newMappingHint,
...
void SkinnedMesh::setDirty(scene::E_BUFFER_TYPE buffer)
{
    for (u32 i=0; i<LocalBuffers.size(); ++i)
...
bool SkinnedMesh::useAnimationFrom(const InterfaceSkinnedMesh *mesh)
{
...
void SkinnedMesh::setInterpolationMode(scene::E_INTERPOLATION_MODE mode)
{
...
void SkinnedMesh::updateBoundingBox(void)
{
    ...
    core::array<scene::SSkinMeshBuffer*> & buffer = *SkinningBuffers;
    BoundingBox.reset(0,0,0);
...
void SkinnedMesh::recoverJointsFromMesh(core::array<scene::IBoneSceneNode*> &jointChildSceneNodes)
{
    for (u32 i=0; i<AllJoints.size(); ++i)
    {
        scene::IBoneSceneNode* node=jointChildSceneNodes[i];
        SJoint *joint=AllJoints[i];
...
void SkinnedMesh::transferJointsToMesh(const core::array<scene::IBoneSceneNode*> &jointChildSceneNodes)
{
    for (u32 i=0; i<AllJoints.size(); ++i)
    {
        const scene::IBoneSceneNode* const node=jointChildSceneNodes[i];
        SJoint *joint=AllJoints[i];
        ...
        joint->GlobalSkinningSpace=(node->getSkinningSpace()==scene::EBSS_GLOBAL);
    }
    // Make sure we recalc the next frame
    LastAnimatedFrame=-1;
    SkinnedLastFrame=false;
...
void SkinnedMesh::transferOnlyJointsHintsToMesh(const core::array<scene::IBoneSceneNode*> &jointChildSceneNodes)
{
    for (u32 i=0; i<AllJoints.size(); ++i)
    {
        const scene::IBoneSceneNode* const node=jointChildSceneNodes[i];
...
void SkinnedMesh::addJoints(core::array<scene::IBoneSceneNode*> &jointChildSceneNodes,
        scene::IAnimatedMeshSceneNode* node, scene::ISceneManager* smgr)
{
    //Create new joints
    for (u32 i=0; i<AllJoints.size(); ++i)
    {
        jointChildSceneNodes.push_back(new BoneSceneNode(0, smgr, 0, i, AllJoints[i]->Name.c_str()));
    }
    ...
    for (u32 i=0; i<jointChildSceneNodes.size(); ++i)
    {
        ...
        scene::IBoneSceneNode* bone=jointChildSceneNodes[i];
        if (parentID!=-1)
