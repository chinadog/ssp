...
//! constructor
DropNode::DropNode(scene::IMesh* mesh, ISceneNode* parent, scene::ISceneManager* mgr, s32 id,
...
//! frame
void DropNode::OnRegisterSceneNode()
{
    if (IsVisible)
    {
...
{
    std::cout << Log::curTimeC() << " RENDER" << std::endl;
...
bool DropNode::removeChild(ISceneNode* child)
{
...
const core::aabbox3d<f32>& DropNode::getBoundingBox() const
{
...
video::SMaterial& DropNode::getMaterial(u32 i)
{
...
u32 DropNode::getMaterialCount() const
{
...
//! Sets a new mesh
void DropNode::setMesh(scene::IMesh* mesh)
{
    if (mesh)
    {
        mesh->grab();
...
scene::IShadowVolumeSceneNode *DropNode::addShadowVolumeSceneNode(const scene::IMesh *shadowMesh, s32 id, bool zfailmethod, f32 infinity)
{
    (void)shadowMesh;
    (void)id;
    (void)zfailmethod;
...
void DropNode::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
{
...
void DropNode::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
{
...
void DropNode::setReadOnlyMaterials(bool readonly)
{
...
bool DropNode::isReadOnlyMaterials() const
{
...
scene::ISceneNode* DropNode::clone(ISceneNode* newParent, scene::ISceneManager* newManager)
{
    if (!newParent)
        newParent = Parent;
    if (!newManager)
    ...
    DropNode* nb = new DropNode(Mesh, newParent,
