...
scene::IMesh * AnimatedMeshSceneNode::getMeshForCurrentFrame()
{
    ...
    else
    {
        ...
        SkinnedMesh* skinnedMesh = reinterpret_cast<SkinnedMesh*>(Mesh);

        if (JointMode == scene::EJUOR_CONTROL)//write to mesh
        ...
        if (JointMode == scene::EJUOR_READ)//read from mesh
        {
        ...
        if(JointMode == scene::EJUOR_CONTROL)
        {
    ...
    if (DebugDataVisible && PassCount==1)
    {
        ...
        if (DebugDataVisible & scene::EDS_SKELETON)
        {
            if (Mesh->getMeshType() == scene::EAMT_SKINNED)
            {
                ...
                for (u32 g=0; g < ((scene::ISkinnedMesh*)Mesh)->getAllJoints().size(); ++g)
                {
                    scene::ISkinnedMesh::SJoint *joint=((scene::ISkinnedMesh*)Mesh)->getAllJoints()[g];
            ...
            if (Mesh->getMeshType() == scene::EAMT_MD3)
            {
                scene::IAnimatedMesh * arrow =
                ...
                scene::IMesh *arrowMesh = arrow->getMesh(0);

                core::matrix4 matr;

                scene::SMD3QuaternionTagList *taglist = ((scene::IAnimatedMeshMD3*)Mesh)->getTagList(
                        (s32)getFrameNr(), 255,
...
s32 AnimatedMeshSceneNode::getStartFrame() const
{
    return StartFrame;
}
...
//! Returns the current start frame number.
s32 AnimatedMeshSceneNode::getEndFrame() const
{
    return EndFrame;
}
...
bool AnimatedMeshSceneNode::setFrameLoop(s32 begin, s32 end)
{
...
scene::IShadowVolumeSceneNode* AnimatedMeshSceneNode::addShadowVolumeSceneNode(
...
scene::IBoneSceneNode* AnimatedMeshSceneNode::getJointNode(const c8* jointName)
{
    ...
    scene::ISkinnedMesh *skinnedMesh=(scene::ISkinnedMesh*)Mesh;
...
scene::ISceneNode* AnimatedMeshSceneNode::getMS3DJointNode(const c8* jointName)
{
...
scene::ISceneNode* AnimatedMeshSceneNode::getXJointNode(const c8* jointName)
{
    return  getJointNode(jointName);
}
...
bool AnimatedMeshSceneNode::setMD2Animation(scene::EMD2_ANIMATION_TYPE anim)
{
    if (!Mesh || Mesh->getMeshType() != scene::EAMT_MD2)
        return false;

    scene::IAnimatedMeshMD2* md = (scene::IAnimatedMeshMD2*)Mesh;
...
bool AnimatedMeshSceneNode::setMD2Animation(const c8* animationName)
{
    if (!Mesh || Mesh->getMeshType() != scene::EAMT_MD2)
        return false;
...
void AnimatedMeshSceneNode::setAnimationEndCallback(scene::IAnimationEndCallBack* callback)
{
...
void AnimatedMeshSceneNode::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
{
...
void AnimatedMeshSceneNode::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
{
...
//! Sets a new mesh
void AnimatedMeshSceneNode::setMesh(scene::IAnimatedMesh* mesh)
{
    if (!mesh)
    ...
    scene::IMesh* m = Mesh->getMesh(0,0);
    if (m)
    {
        Materials.clear();
...
const scene::SMD3QuaternionTag* AnimatedMeshSceneNode::getMD3TagTransformation(const core::stringc& tagname)
{
...
void AnimatedMeshSceneNode::updateAbsolutePosition()
{
    ...
    if (!Mesh || Mesh->getMeshType() != scene::EAMT_MD3)
        return;

    scene::SMD3QuaternionTagList *taglist;
    taglist = ( (scene::IAnimatedMeshMD3*) Mesh )->getTagList ( (s32)getFrameNr(),255,getStartFrame (),getEndFrame () );
    if (taglist)
    {
        if (!MD3Special)
        {
        ...
        scene::SMD3QuaternionTag parent ( MD3Special->Tagname );
        if (Parent && Parent->getType() == scene::ESNT_ANIMATED_MESH)
        {
            const scene::SMD3QuaternionTag * p = ((IAnimatedMeshSceneNode*) Parent)->getMD3TagTransformation
                                    ( MD3Special->Tagname );
        ...
        scene::SMD3QuaternionTag relative( RelativeTranslation, RelativeRotation );
...
void AnimatedMeshSceneNode::setJointMode(scene::E_JOINT_UPDATE_ON_RENDER mode)
{
    checkJoints();
    JointMode=mode;
}
...
void AnimatedMeshSceneNode::setTransitionTime(f32 time)
{
    ...
    if (ttime != 0)
        setJointMode(scene::EJUOR_CONTROL);
    else
        setJointMode(scene::EJUOR_NONE);
}
...
/*!
*/
scene::ISceneNode* AnimatedMeshSceneNode::clone(ISceneNode* newParent, scene::ISceneManager* newManager)
{
    if (!newParent)
        newParent = Parent;
    if (!newManager)
