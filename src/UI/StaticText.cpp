...
//! constructor
StaticText::StaticText(IrrlichtDevice* device,
        const wchar_t* text, bool border,
            gui::IGUIEnvironment* environment, IGUIElement* parent,
            s32 id, const core::rect<s32>& rectangle,
            bool background)
: IGUIStaticText(environment, parent, id, rectangle),
    HAlign(gui::EGUIA_UPPERLEFT), VAlign(gui::EGUIA_UPPERLEFT),
...
//        Parent->removeChild(this);
    ...
    if (Background)
    {
        if ( !OverrideBGColorEnabled )	// skin-colors can change
        {
            ...
            BGColor = video::SColor(205,20,20,20);
        }
    ...
    if (Border)
    {
        skin->draw3DSunkenPane(this, video::SColor(255,255,0,0), true, false, frameRect, &AbsoluteClippingRect);
    ...
    if (Text.size())
    {
        gui::IGUIFont* font = getActiveFont();

        if (font)
        {
            if (!WordWrap)
            {
                if (VAlign == gui::EGUIA_LOWERRIGHT)
                {
                ...
                font->draw(Text.c_str(), frameRect,
                    OverrideColorEnabled ? OverrideColor : skin->getColor(isEnabled() ? gui::EGDC_BUTTON_TEXT : gui::EGDC_GRAY_TEXT),
                    HAlign == gui::EGUIA_CENTER, VAlign == gui::EGUIA_CENTER, (RestrainTextInside ? &AbsoluteClippingRect : NULL));
            }
            else
            {
...
r.UpperLeftCorner.Y += 10;
                    font->draw(BrokenText[i].c_str(), r,
                        OverrideColorEnabled ? OverrideColor : skin->getColor(isEnabled() ? gui::EGDC_BUTTON_TEXT : gui::EGDC_GRAY_TEXT),
//                        video::SColor(255,255,255,255),
                               HAlign == gui::EGUIA_CENTER, false, (RestrainTextInside ? &AbsoluteClippingRect : NULL));
...
void StaticText::setOverrideFont(gui::IGUIFont* font)
{
...
gui::IGUIFont * StaticText::getOverrideFont() const
{
    return OverrideFont;
}

//! Get the font which is used right now for drawing
gui::IGUIFont* StaticText::getActiveFont() const
{
    if ( OverrideFont )
        return OverrideFont;
    gui::IGUISkin* skin = Environment->getSkin();
...
void StaticText::setTextAlignment(gui::EGUI_ALIGNMENT horizontal, gui::EGUI_ALIGNMENT vertical)
{
    HAlign = horizontal;
    VAlign = vertical;
}
...
void StaticText::breakText()
{
    ...
    if (Border)
        elWidth -= 2*skin->getSize(gui::EGDS_TEXT_DISTANCE_X);
    wchar_t c;
...
void StaticText::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options=0) const
{
    ...
    out->addEnum	("HTextAlign",          HAlign, gui::GUIAlignmentNames);
    out->addEnum	("VTextAlign",          VAlign, gui::GUIAlignmentNames);
...
void StaticText::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options=0)
{
    ...
    setTextAlignment( (gui::EGUI_ALIGNMENT) in->getAttributeAsEnumeration("HTextAlign", gui::GUIAlignmentNames),
                      (gui::EGUI_ALIGNMENT) in->getAttributeAsEnumeration("VTextAlign", gui::GUIAlignmentNames));

    // OverrideFont = in->getAttributeAsFont("OverrideFont");
}

void StaticText::setLifeTime(u32 delay, u32 value)
{
    m_startTime = m_device->getTimer()->getTime() + delay;
